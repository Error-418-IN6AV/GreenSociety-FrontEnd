...
export const DetallePage = () => {
  ...
  const deleteDetail = async (_id) => {
    try {
      Swal.fire({
        title: 'Are you sure to delete this Donation Detail',
        icon: 'question',
        showConfirmButton: true,
        showDenyButton: true,
      }).then(async (confirmDelete) => {
        if (confirmDelete.isConfirmed) {
          const { data } = await axios.delete(`http://localhost:3000/detalledonaciones/delete/${_id}`, { headers: headers }).catch(
            (err) => {
