...
export const PageCategory = () => {
  ...
  const addCategory = async (e) => {
    try {
      e.preventDefault();
      const { data } = await axios.post('http://localhost:3000/category/add', formData)
            ...
            categories.map(({ _id, name, description }, i) => {
              ...
              const update = async (id) => {
                try {
                  ...
                  const { data } = await axios.put(`http://localhost:3000/category/update/${idCategory}`, categoryUpdate)
                  if(data.message){
                    Swal.fire({
