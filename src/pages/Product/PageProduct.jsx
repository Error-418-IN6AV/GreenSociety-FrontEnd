...
export const PageProduct = () => {
    ...
    const getProducts = async () => {
        try {
            const { data } = await axios('http://localhost:3000/product/getProducts')
            if (data.products) {
    ...
    const getCategories = async () => {
        try {
            const { data } = await axios('http://localhost:3000/category/getCategories')
            if (data.categories) {
        ...
        try {
            Swal.fire({
                ...
                if (confirmDelete.isConfirmed) {
                    const { data } = await axios.delete(`http://localhost:3000/product/delete/${id}`).catch(
                        (err) => {
    ...
    const search = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post('http://localhost:3000/product/search', getSearch)
